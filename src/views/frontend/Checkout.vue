        ...
        <form class="row justify-content-md-center" @submit.prevent="payOrder(times)">
                  ...
                  <tr v-for="item in order.products" :key="item.id">
                    ...
                    <td class="text-left">
                      <p>
                        {{ item.product.title }}
                        <br />
                        NT${{item.product.price}} x {{ item.qty }} {{item.product.unit}}
                      </p>
                    </td>
                  </tr>
                </tbody>
          ...
          <div class="col-lg-8 col-md-6">
            <div class="p-2">
              ...
              <div class="my-3 px-lg-5 down" :class="{'d-none':times!==3}">
                ...
                <table class="table table-striped text-dark m-0">
                  <tbody>
                    ...
                    <tr>
                      <th>付款狀態 :</th>
                      <td class="text-danger" v-if="paymethods==1&&order.is_paid">已選擇貨到付款</td>
                      <td class="text-danger" v-else-if="paymethods==2&&order.is_paid">已選擇ATM 匯款</td>
              ...
              <button class="btn btn-danger my-3" @click="$router.push('/')">再去逛逛</button>
            </div>
          </div>
        </form>
      </div>
