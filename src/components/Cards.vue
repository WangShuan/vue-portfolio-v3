    ...
    <div class="row d-inline-flex text-dark">
      <div class="col-lg-4 col-md-6 mb-4" v-for="item in products" :key="item.id">
        <div class="card border-0 shadow-sm">
          ...
          <div class="card-body">
            <h5 class="card-title text-darken m-0">{{ item.title }}</h5>
            <span class="badge badge-info p-1">{{item.category}}</span>
            <p class="card-text">{{ item.content }}</p>
            <div class="d-flex justify-content-between align-items-baseline">
              <div class="h5" v-if="!item.price">{{ item.origin_price }} 元</div>
              <del class="h6 text-muted m-auto" v-if="item.price">原價${{ item.origin_price }}</del>
              <div class="h5 text-danger m-auto" v-if="item.price">現在只要${{ item.price }}</div>
            </div>
          </div>
